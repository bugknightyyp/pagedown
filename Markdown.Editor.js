...
(function () {
    ...
    function CommandManager(pluginHooks, getString, converter, wrapImageInLink, convertImagesToLinks) {
        this.hooks = pluginHooks;// 钩子
        this.getString = getString;// defaultStrings 命令描述
        this.converter = converter; // markdown to html converter
        this.wrapImageInLink = wrapImageInLink; // 把图片打包进链接里
        this.convertImagesToLinks = convertImagesToLinks; // 图片转换成链接
    }
    ...
    commandProto.stripLinkDefs = function (text, defsToAdd) {
        ...
        text = text.replace(/^[ ]{0,3}\[(\d+)\]:[ \t]*\n?[ \t]*<?(\S+?)>?[ \t]*\n?[ \t]*(?:(\n*)["(](.+?)[")][ \t]*)?(?:\n+|$)/gm,
            function (totalMatch, id, link, newlines, title) {
                ...
                if (newlines) {// 如果url 和 title 之间有换行，则title的不是url对应的title，而是内容的一部分
